						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center">研究方向</th>
									<th class="center">已发表论文</th>
									<th class="center">年龄</th>
									<th class="center">职称</th>
									<th class="center">性别</th>
									<th class="center">学术兼职</th>
								...
								<c:when test="${not empty userList}">
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										...
										<tr>
											...
											<td class='center'>${user.XUELI}</td>
											<td class='center'>${user.FANGXIANG}</td>
											<td class='center'>${user.PUBPAPER}								
											<c:forEach items="${lunwenList}" var="lunwen">																									
												<c:if test="${user.NUMBER==lunwen.FIRSTAUTHORNUM ||user.NUMBER==lunwen.SECONDAUTHERNUM ||user.NUMBER==lunwen.THIRDAUTHORNUM}">
													《<a onclick="viewLunwen('${lunwen.LUNWEN_ID}')" style="cursor:pointer;">${lunwen.LWNAME}</a>》
												</c:if>
											</c:forEach>																														
											</td>
											<!-- ${user.PUBPAPER}- -->
											<td class='center'>${user.AGE}</td>
											<td class='center'>${user.ZHICHENG}</td>
											<td style="width: 60px;" class="center">
												<c:if test="${user.SEX == '0' }">女</c:if>
												<c:if test="${user.SEX == '1' }">男</c:if>
											</td>
																					
											<td class='center'>${user.XSJZ}</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="下载" onclick="persontoExcel('${user.USER_ID}');" >
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="window.location.href='<%=basePath%>/user/persontoExcel.do?USER_ID=${user.USER_ID}'" class="tooltip-success" data-rel="tooltip" title="下载">
																	<span class="green">
...
function toExcel(){
	...
	window.location.href='<%=basePath%>user/excel.do?keywords='+keywords+'&ROLE_ID='+ROLE_ID;
}


//导出个人所有科研成果excel
function persontoExcel(user_id){
	window.location.href='<%=basePath%>user/persontoExcel.do?USER_ID='+user_id;
}
//打开上传excel页面
function fromExcel(){
	 top.jzts();
...
//查看论文信息
function viewLunwen(LUNWEN_ID){
	
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="论文信息";
	 diag.URL = '<%=basePath%>lunwen/view.do?LUNWEN_ID='+LUNWEN_ID;
	 diag.Width = 469;
	 diag.Height = 380;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
